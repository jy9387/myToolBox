function [ patch_avrg ] = extr_token( rc_token, rc_img_idx, patch_size, gt_root, save_root, img_root )
...
if ~exist([save_root, '/img/'], 'dir'), mkdir([save_root, '/img/']); end;
if ~exist([save_root, '/gt/'], 'dir'), mkdir([save_root, '/gt/']); end;

patch_avrg = zeros(patch_size);
rc_img_idx_unq = unique(rc_img_idx);
idx = 0;
for i = 1:length(rc_img_idx_unq)
    img = imread([img_root, num2str(rc_img_idx_unq(i)), '.jpg']);
...
%         save([save_root, '/patch_', num2str(idx), '.mat'], 'patch');
        ...
        img_patch = img_pad(patch_r_min:patch_r_max, patch_c_min: patch_c_max, :);
        imwrite(img_patch, [save_root, '/img/', num2str(rc_img_idx_unq(i)), '_img_', num2str(p), '.png']);
        imwrite(patch, [save_root, '/gt/', num2str(rc_img_idx_unq(i)), '_gt_', num2str(p), '.png'])
    end
end
