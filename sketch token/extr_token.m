...
for i = 1:length(rc_img_idx_unq)
    ...
    for p = 1:size(patches_yx, 1)
        idx = idx + 1;
        patch_r_min = patches_yx(p, 1);
        patch_r_max = patches_yx(p, 3);
        patch_c_min = patches_yx(p, 2);
        patch_c_max = patches_yx(p, 4);        
        patch = gt(patch_r_min:patch_r_max, patch_c_min: patch_c_max);
