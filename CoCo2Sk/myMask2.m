function [ mask ] = myMask2( anns, I )
    ...
    if( any(isfield(anns,{'segmentation','bbox'})) )
        ...
        close; figure(1); 
        I2 = zeros(size(I,1),size(I,2)); imshow(I2,'border','tight','initialmagnification','fit'); hold on;
        set(1, 'position', [0 0 size(I, 2) size(I, 1)]);
        S={anns.segmentation}; hs=zeros(10000,1); k=0;
        pFill={'LineWidth',1};
        for i=1:n
